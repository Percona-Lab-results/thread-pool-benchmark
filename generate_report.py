...
function traceColor(i, n, alpha) {
  ...
  // vivid tones in the spirit of the classic d3/Plotly palette
...
function buildTraces() {
  ...
  const lineTraces = series.map((s, i) => ({
    ...
    marker: { size: 7, color: traceColor(i, series.length) },
    hovertemplate:
  ...
  const centerTraces = series.map((s, i) => ({
    ...
    x: s.pts.map(p=>p.threads),
    y: s.pts.map(p=>p[metric]),
    marker: { size: 3, color: "rgba(255, 255, 255, 0.65)" },
    legendgroup: s.name,
    showlegend: false,
    hoverinfo: "skip",
    visible: HIDDEN.has(s.name) ? "legendonly" : true,
  ...
  // Two permanent, initially empty traces for the hover highlight. They are
  // updated in place with Plotly.restyle, which is far cheaper than adding
  // and deleting traces on every hover (that caused page freezes).
  const highlightTraces = [
    { type: "scatter", mode: "lines", x: [], y: [],
      line: { color: "#ffffff", width: 7 },
      meta: "hl-halo", hoverinfo: "skip", showlegend: false },
    { type: "scatter", mode: "lines+markers", x: [], y: [],
      line: { color: "#000000", width: 2 },
      marker: { size: 8, color: "#000000",
                line: { color: "#ffffff", width: 1.5 } },
      meta: "hl-top", hoverinfo: "skip", showlegend: false },
  ];

  // Error circles first so the lines and markers draw on top of them,
  // centre dots last so they stay on top of the node markers
  return [...errorTraces, ...lineTraces, ...centerTraces, ...highlightTraces];
}
...
function attachPlotlyClick() {
  ...
  // Highlight the hovered series by drawing it into two pre-allocated
  // traces (white border line + bright copy on top). Hover/unhover events
  // only arm a timer; the single Plotly.restyle happens after the pointer
  // settles, so sweeping across a busy region costs nothing and cannot
  // freeze the page.
  const HOVER_APPLY_MS = 120;
  let hoverTimer = null;
  let pendingGroup = null;   // group to apply, null = clear
  let appliedGroup = null;   // group currently drawn

  function highlightIdx() {
    const data = chartDiv.data || [];
    const halo = data.findIndex(t => t.meta === "hl-halo");
    const top = data.findIndex(t => t.meta === "hl-top");
    return halo >= 0 && top >= 0 ? [halo, top] : null;
  }

  function applyPending() {
    hoverTimer = null;
    if (pendingGroup === appliedGroup) return;
    const idx = highlightIdx();
    if (!idx) return;
    if (pendingGroup === null) {
      appliedGroup = null;
      Plotly.restyle(chartDiv, { x: [[], []], y: [[], []] }, idx);
      return;
    }
    const src = (chartDiv.data || []).find(
      t => t.legendgroup === pendingGroup && t.name);
    if (!src) return;
    appliedGroup = pendingGroup;
    // Brighter version of the series color (full saturation, mid lightness)
    // so the highlighted line pops next to the muted palette
    const bright = src.line.color.replace(
      /hsl\((\d+), \d+%, \d+%\)/, "hsl($1, 90%, 48%)");
    Plotly.restyle(chartDiv, {
      x: [src.x, src.x],
      y: [src.y, src.y],
      "line.color": ["#ffffff", bright],
      "marker.color": ["#ffffff", bright],
    }, idx);
  }
  ...
  function schedule(group) {
    ...
    hoverTimer = setTimeout(applyPending, HOVER_APPLY_MS);
  }
