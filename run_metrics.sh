...
generate_config() {
    ...
    echo "max_prepared_stmt_count         = 10000000" >> "$CFG"
    echo "thread_stack                    = 512K" >> "$CFG"
    echo "thread_cache_size               = 10000" >> "$CFG"
    echo "back_log                        = 16384" >> "$CFG"
    ...
    if [ "$TP_CONF" == "off" ]; then
        ...
        if [[ "${DBMS_NAME,,}" == percona* ]]; then
            ...
            echo "thread_pool_size                = $TP_SIZE" >> "$CFG"
            echo "thread_pool_max_active_query_threads = 12000       # ~ Percona thread_pool_max_threads" >> "$CFG"
